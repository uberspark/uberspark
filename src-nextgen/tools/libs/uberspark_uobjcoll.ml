...
(*type Uberspark_codegen.Uobjcoll.uobjs_publicmethod_info_t =
{
...
let d_hdr_mf_node: Uberspark_manifest.Uobjcoll.uobjcoll_hdr_t = {f_namespace = ""; f_platform = ""; f_arch = ""; f_cpu = ""; f_hpl = ""; f_intrauobjcoll_sentinels = [];};;
...
let d_uobjs_publicmethods_hashtbl = ((Hashtbl.create 32) : ((string, Uberspark_codegen.Uobjcoll.uobjs_publicmethod_info_t)  Hashtbl.t));; 
...
let parse_manifest 
	...
	let rval = (Uberspark_manifest.Uobjcoll.parse_uobjcoll_hdr mf_json d_hdr_mf_node ) in
	if (rval == false) then (false)
	else

	let dummy=0 in begin
		d_path_ns := !Uberspark_namespace.namespace_root_dir  ^ "/" ^ d_hdr_mf_node.f_namespace;
...
(*	let rval_assoc = (Uberspark_manifest.Uobjcoll.parse_uobjcoll_interuobjcoll_publicmethods_into_assoc_list mf_json d_uobjcoll_interuobjcoll_publicmethods_assoc_list_mforder) in*)
(*	if (rval_hashtbl == false) || (rval_assoc == false) then (false)*)
	let rval = (Uberspark_manifest.Uobjcoll.parse_uobjcoll_interuobjcoll_publicmethods_into_assoc_list mf_json d_uobjcoll_interuobjcoll_publicmethods_assoc_list_mforder) in
	if (rval == false) then (false)
	else

	let dummy=0 in begin
		Uberspark_logger.log ~lvl:Uberspark_logger.Debug "parsed uobjcoll-interuobjcoll-publicmethods: total=%u" 
			(List.length !d_uobjcoll_interuobjcoll_publicmethods_assoc_list_mforder);
	end;
...
(* sentinel_facet = interuobjcoll or intrauobjcoll *)
...
let get_sentinel_info_for_sentinel_facet_and_type
	...
	let sentinel_mf_filename = (!Uberspark_namespace.namespace_root_dir ^ "/" ^ 
		...
		d_hdr_mf_node.f_arch ^ "/" ^ d_hdr_mf_node.f_cpu ^ "/" ^ d_hdr_mf_node.f_hpl ^ "/" ^ sentinel_facet ^ "/" ^ 
...
(*--------------------------------------------------------------------------*)
(* create uobj collection interuobjcoll and intrauobjcoll sentinels hashtbl *)
(*--------------------------------------------------------------------------*)
let create_interuobjcoll_intrauobjcoll_sentinels_hashtbl
	()
	: bool = 
	...
	List.iter ( fun ( (canonical_pm_name:string), (pm_sentinel_info: Uberspark_manifest.Uobjcoll.uobjcoll_interuobjcoll_publicmethods_t)) -> 
	...
	) !d_uobjcoll_interuobjcoll_publicmethods_assoc_list_mforder;
	Uberspark_logger.log ~lvl:Uberspark_logger.Debug "create_sentinels_hashtbl: total unique interuobjcoll sentinels=%u" 
		(Hashtbl.length sentinel_type_to_sentinel_facet);


	(* now iterate over the interuobjcoll sentinel type to sentinel facet hashtbl, get the corresponding 
	sentinel info and add it to d_uobjcoll_interuobjcoll_sentinels_hashtbl *)
	Hashtbl.iter (fun (sentinel_type:string) (sentinel_facet:string)  ->
		if !retval then begin
			...
			if (rval == false) then begin
				...
				Hashtbl.add d_uobjcoll_interuobjcoll_sentinels_hashtbl sentinel_type sinfo;
			end;
		end;
	...
	let dummy=0 in begin
	Uberspark_logger.log ~lvl:Uberspark_logger.Debug "create_sentinels_hashtbl: total unique interuobjcoll sentinels=%u" 
		(Hashtbl.length d_uobjcoll_interuobjcoll_sentinels_hashtbl);
	...
	) d_hdr_mf_node.f_intrauobjcoll_sentinels;
...
let initialize_uobjs_baseinfo 
	...
	if not (d_uobjcoll_uobjs_mf_node.f_prime_uobj_ns = "") then begin
			...
			if (rval) then begin
				...
				if (Uberspark_namespace.is_uobj_ns_in_uobjcoll_ns d_uobjcoll_uobjs_mf_node.f_prime_uobj_ns
					d_hdr_mf_node.f_namespace) then begin
	...
	List.iter (fun templar_uobj_ns ->
			...
			if (rval) && !retval then begin
				...
				if (Uberspark_namespace.is_uobj_ns_in_uobjcoll_ns templar_uobj_ns d_hdr_mf_node.f_namespace) then begin
					uobjinfo_entry.f_uobjinfo.f_uobj_is_incollection <- true;
				end else begin
					uobjinfo_entry.f_uobjinfo.f_uobj_is_incollection <- false;
				end;
...
let consolidate_sections_with_memory_map
	...
	List.iter ( fun ( (pm_name:string), (pm_sentinel_info:Uberspark_manifest.Uobjcoll.uobjcoll_interuobjcoll_publicmethods_t))  ->
		...
		List.iter ( fun (sentinel_type:string) ->
			...
			let sentinel_info = Hashtbl.find d_uobjcoll_interuobjcoll_sentinels_hashtbl sentinel_type in
	...
	(* 
	) !d_interuobjcoll_sentinels_list_mforder; *)
	...
	List.iter (fun ((pm_name:string) ,(pm_info:Uberspark_codegen.Uobjcoll.uobjs_publicmethod_info_t))  ->
		Uberspark_logger.log ~lvl:Uberspark_logger.Debug "pm_name=%s" pm_name;
	...
	(*
	) !d_intrauobjcoll_sentinels_list_mforder;
	*)
...
let create_uobjs_publicmethods_hashtbl
	(publicmethods_hashtbl : ((string, Uberspark_codegen.Uobjcoll.uobjs_publicmethod_info_t)  Hashtbl.t))
	: unit =
	...
	Hashtbl.iter (fun (canonical_pm_name:string) (entry:Uberspark_codegen.Uobjcoll.uobjs_publicmethod_info_t)  ->
		Uberspark_logger.log ~lvl:Uberspark_logger.Debug "canonical pm_name=%s; pm_name=%s, pm_addr=0x%08x" 
			canonical_pm_name entry.f_uobjpminfo.f_name entry.f_uobjpminfo.f_addr; 
...
let prepare_namespace_for_build
	...
	let uobjcoll_canonical_namespace = d_hdr_mf_node.f_namespace in
...
let build
	...
	let dummy = 0 in begin
	d_hdr_mf_node.f_arch <- d_target_def.f_arch;
	d_hdr_mf_node.f_cpu <- d_target_def.f_cpu;
	d_hdr_mf_node.f_platform <- d_target_def.f_platform;
	end;

    (* create intra uobjcoll sentinels list *)
	(* TO REMOVE *)
	let rval = (create_sentinels_list "intrauobjcoll" d_uobjcoll_sentinels_mf_node.f_intrauobjcoll d_intrauobjcoll_sentinels_list_mforder) in	
    if (rval == false) then	begin
	...
	let dummy = 0 in begin
	Uberspark_logger.log "created intra-uobjcoll sentinel list, total sentinels=%u" (List.length !d_intrauobjcoll_sentinels_list_mforder);
	end;

    (* create inter uobjcoll sentinels list *)
	(* TO REMOVE *)
	let rval = (create_sentinels_list "interuobjcoll" d_uobjcoll_sentinels_mf_node.f_interuobjcoll d_interuobjcoll_sentinels_list_mforder) in	
    if (rval == false) then	begin
	...
	let dummy = 0 in begin
	Uberspark_logger.log "created inter-uobjcoll sentinel list, total sentinels=%u" (List.length !d_interuobjcoll_sentinels_list_mforder);
	end;


	(*create interuobjcoll and intrauobjcoll sentinels hashtbl *)
	let rval = (create_interuobjcoll_intrauobjcoll_sentinels_hashtbl ()) in 
	...
	let dummy = 0 in begin
	Uberspark_logger.log "created uobj collection interuobjcoll and intrauobjcoll sentinels hashtbl";
	end;
