...
class uobject 
	...
	val d_load_addr = ref Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_image_load_address;
	...
	val d_size = ref Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_image_size; 
	method get_d_size = !d_size;
	method set_d_size size = (d_size := size);

	(* uobj binary image uniform size flag *)
	val d_uniform_size = ref Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_image_uniform_size; 
	...
	val d_alignment = ref Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_image_alignment; 
	...
	method consolidate_sections_with_memory_map
		...
		*)
		...
		List.iter (fun (key, (x:Uberspark.Defs.Basedefs.section_info_t))  ->
			...
			let remainder_size = (x.usbinformat.f_size mod Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_section_alignment) in
			let padding_size = ref 0 in
				if remainder_size > 0 then
					begin
						padding_size := Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_section_alignment - remainder_size;
					end
				else
					begin
						padding_size := 0;
			...
			d_memorymapped_sections_list := !d_memorymapped_sections_list @ [ (key, 
				{ fn_name = x.fn_name;	
					...
					usbinformat = { f_type=x.usbinformat.f_type; 
													f_prot=0; 
													f_size = section_size;
													f_aligned_at = Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_section_alignment; 
													f_pad_to = Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_section_alignment; 
		...
		if (self#get_d_uniform_size) then begin
			...
			if (!uobj_section_load_addr mod self#get_d_alignment) > 0 then begin
				(* uobj_section_load_addr is __not__ aligned at uobj_image_alignment *)
				(* add padding section *)
				d_memorymapped_sections_list := !d_memorymapped_sections_list @ [ ("usuobj_padding", 
				...
				(* uobj_section_load_addr is aligned at uobj_image_alignment *)
	...
	method add_default_uobj_binary_sections	
		...
		l_add_section "uobj_ssa" [ ".uobj_ssa" ] 
					...
					0 
					Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_default_section_size
					Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_section_alignment
					Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_section_alignment;
	...
	method prepare_sources
		...
		json_node_uberspark_uobj_var.sources <- !l_uobjrtl_sources_c_files @ 
			json_node_uberspark_uobj_var.sources;

		(* add uobjrtl casm source files to the list of casm sources *)
		json_node_uberspark_uobj_var.sources <- !l_uobjrtl_sources_casm_files @ 
			json_node_uberspark_uobj_var.sources;
