...
class uobject 
	...
	val d_load_addr = ref Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_binary_image_load_address;
	...
	val d_size = ref Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_binary_image_size; 
	method get_d_size = !d_size;
	method set_d_size size = (d_size := size);

	(* uobj binary image uniform size flag *)
	val d_uniform_size = ref Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_binary_image_uniform_size; 
	...
	val d_alignment = ref Uberspark.Platform.json_node_uberspark_platform_var.binary.uobj_binary_image_alignment; 
	...
	method consolidate_sections_with_memory_map
		...
		*)
		...
		List.iter (fun (key, (x:Uberspark.Defs.Basedefs.section_info_t))  ->
			...
			let remainder_size = (x.usbinformat.f_size mod Uberspark.Platform.json_node_uberspark_platform_var.binary.binary_uobj_section_alignment) in
			let padding_size = ref 0 in
				if remainder_size > 0 then
					begin
						padding_size := Uberspark.Platform.json_node_uberspark_platform_var.binary.binary_uobj_section_alignment - remainder_size;
					end
				else
					begin
						padding_size := 0;
	...
	method add_default_uobj_binary_sections	
		...
		l_add_section "uobj_ssa" [ ".uobj_ssa" ] 
					...
					0 
					Uberspark.Platform.json_node_uberspark_platform_var.binary.binary_uobj_default_section_size
					Uberspark.Platform.json_node_uberspark_platform_var.binary.binary_uobj_section_alignment
					Uberspark.Platform.json_node_uberspark_platform_var.binary.binary_uobj_section_alignment;
