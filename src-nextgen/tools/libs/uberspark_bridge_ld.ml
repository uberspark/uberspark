...
let load 
	...
	let bridge_ns_json_path = !Uberspark_config.namespace_root_dir ^ "/" ^ Uberspark_config.namespace_root ^ "/" ^
		Uberspark_config.namespace_bridge_ld_bridge ^ "/" ^ bridge_ns ^ "/" ^
...
let store 
	...
	let bridge_ns_json_path = !Uberspark_config.namespace_root_dir ^ "/" ^ Uberspark_config.namespace_root ^ "/" ^
		Uberspark_config.namespace_bridge_ld_bridge ^ "/" ^ bridge_ns in
...
let build 
	...
	if bridge_ld.bridge_hdr.btype = "container" then
		begin
			let bridge_ns = Uberspark_config.namespace_bridge_ld_bridge ^ "/" ^
