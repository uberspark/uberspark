...
let store_settings_to_namespace
	...
	List.iter (fun (x) ->
		match x with 
		| "cc-bridge" ->
			let output_json_file = Uberspark_config.namespace_root ^ !hdr_namespace ^ "/" ^ 
								cc_bridge_settings.hdr.namespace ^ "/uberspark-bridge.json" in
			
			let oc = open_out output_json_file in
