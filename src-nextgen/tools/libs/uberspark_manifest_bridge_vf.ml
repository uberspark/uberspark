...
let json_node_uberspark_bridge_vf_to_var
	...
	(json_node_uberspark_bridge_vf_var : json_node_uberspark_bridge_t) 
	: bool =
	let retval = ref false in

	try
		let open Yojson.Basic.Util in
			let rval = json_node_bridge_hdr_to_var mf_json json_node_uberspark_bridge_vf_var in

			if rval then begin
				json_node_uberspark_bridge_vf_var.bridge_cmd <- json_list_to_string_list ( Yojson.Basic.Util.to_list (Yojson.Basic.Util.member "uberspark.bridge.bridge_cmd" mf_json));
...
let json_node_uberspark_bridge_vf_var_to_jsonstr  
	...
	retstr := !retstr ^ Printf.sprintf  "\n";
	retstr := !retstr ^ (json_node_bridge_hdr_var_to_jsonstr json_node_uberspark_bridge_vf_var) ^ ",";
