...
let generate_sentinel_code	
    ...
    Uberspark.Logger.log ~lvl:Uberspark.Logger.Debug "%s: length of pm_sentinel_list=%u" 
...
let generate_sentinel_libcode	
    ...
    Uberspark.Logger.log ~lvl:Uberspark.Logger.Debug "%s: length of canonical_publicmethods_sentinels_hashtbl=%u" 
...
let generate_uobj_binary_image_section_mapping	
	...
	(uobjcoll_uobjinfo_list : Uberspark.Defs.Basedefs.uobjinfo_t list)
    : bool	= 
        let retval = ref false in
        
        Uberspark.Logger.log ~lvl:Uberspark.Logger.Debug "uobjcoll_uobjinfo_list length=%u" (List.length uobjcoll_uobjinfo_list);
        let oc = open_out output_filename in
            Printf.fprintf oc "\n/* --- this file is autogenerated --- */";
            Printf.fprintf oc "\n/* %s */" output_banner;
            Printf.fprintf oc "\n/* author: amit vasudevan (amitvasudevan@acm.org) */";
            ...
            List.iter ( fun (uobjinfo_entry : Uberspark.Defs.Basedefs.uobjinfo_t) ->
                Printf.fprintf oc "\n";
                Printf.fprintf oc "\n.section .section_uobj_%s" uobjinfo_entry.f_uobj_name;
                Printf.fprintf oc "\n.incbin \"%s\"" ("./" ^ uobjinfo_entry.f_uobj_name ^ "/" ^ Uberspark.Namespace.namespace_uobj_build_dir ^ "/" ^ Uberspark.Namespace.namespace_uobj_binary_flat_image_filename);
                Printf.fprintf oc "\n";
...
let generate_linker_script 
    ...
    let oc = open_out output_filename in
		...
		(*List.iter (fun (key, (x:Uberspark.Defs.BaseUberspark.Defs.section_info_t))  ->
                (* new section memory *)
                Printf.fprintf oc "\n %s (%s) : ORIGIN = 0x%08x, LENGTH = 0x%08x"
                    ("mem_" ^ x.fn_name)
   ...
   (*     List.iter ( fun ( (key:string), (section_info:Uberspark.Defs.BaseUberspark.Defs.section_info_t)) ->
    ...
    *)
         ...
         List.iter ( fun ( (key:string), (section_info:Uberspark.Defs.Basedefs.section_info_t)) ->
...
let generate_top_level_include_header 
    ...
    (configdefs_assoc_list : ((string * Uberspark.Manifest.Uobjcoll.json_node_uberspark_uobjcoll_configdefs_t) list) ) 
    : unit   =
    ...
    let oc = open_out output_filename in
    	...
    	List.iter ( fun ( (name : string) , (entry : Uberspark.Manifest.Uobjcoll.json_node_uberspark_uobjcoll_configdefs_t) ) -> 
