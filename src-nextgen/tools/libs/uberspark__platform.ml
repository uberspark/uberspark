...
let settings_save 
	...
	saved_json_node_uberspark_config_var.casm_bridge_namespace <- json_node_uberspark_config_var.casm_bridge_namespace;
	saved_json_node_uberspark_config_var.ld_bridge_namespace <- json_node_uberspark_config_var.ld_bridge_namespace;
	saved_json_node_uberspark_config_var.uberspark_vf_bridge_namespace <- json_node_uberspark_config_var.uberspark_vf_bridge_namespace;
...
let settings_restore 
	...
	json_node_uberspark_config_var.casm_bridge_namespace <- saved_json_node_uberspark_config_var.casm_bridge_namespace;
	json_node_uberspark_config_var.ld_bridge_namespace <- saved_json_node_uberspark_config_var.ld_bridge_namespace;
	json_node_uberspark_config_var.uberspark_vf_bridge_namespace <- saved_json_node_uberspark_config_var.uberspark_vf_bridge_namespace;
...
let load 
	...
	let config_ns_json_path = (Uberspark.Namespace.get_namespace_staging_dir_prefix ()) ^ "/" ^ 
		Uberspark.Namespace.namespace_root ^ "/" ^ Uberspark.Namespace.namespace_config ^ "/" ^ 
	...
	(*let (rval, l_json_node_uberspark_manifest, json_node_uberspark_config) = 
		...
		Uberspark.Namespace.namespace_config_mf_node_type_tag in*)
...
let settings_get 
	...
	match setting_name with
		...
		| "ld_bridge_namespace" -> settings_value := (Printf.sprintf "%s" json_node_uberspark_config_var.ld_bridge_namespace);
		| "uberspark_vf_bridge_namespace" -> settings_value := (Printf.sprintf "%s" json_node_uberspark_config_var.uberspark_vf_bridge_namespace);
		| _ -> retstatus := false;
	;
...
let settings_set 
	...
	match setting_name with
		...
		| "ld_bridge_namespace" -> json_node_uberspark_config_var.ld_bridge_namespace <- setting_value;
		| "uberspark_vf_bridge_namespace" -> json_node_uberspark_config_var.uberspark_vf_bridge_namespace <- setting_value;
		| _ -> retval := false;
	;
...
let create_from_file
	...
	let output_config_dir = ((Uberspark.Namespace.get_namespace_staging_dir_prefix ()) ^ "/" ^ Uberspark.Namespace.namespace_root ^ "/" ^ Uberspark.Namespace.namespace_config) in
	let output_config_json_pathname = output_config_dir ^ "/" ^ Uberspark.Namespace.namespace_config_mf_filename in
...
let create_from_existing_ns
	...
	let output_config_dir = (!Uberspark.Namespace.namespace_root_dir ^ "/" ^ Uberspark.Namespace.namespace_root ^ "/" ^ Uberspark.Namespace.namespace_config ^ "/" ^ output_config_name) in
	let output_config_json_pathname = output_config_dir ^ "/" ^ Uberspark.Namespace.namespace_config_mf_filename in
...
let switch 
	...
	let retval = ref true in
	let config_ns_path = !Uberspark.Namespace.namespace_root_dir ^ "/" ^ Uberspark.Namespace.namespace_root ^ "/" ^ Uberspark.Namespace.namespace_config ^ "/" ^ config_name in 
	let config_ns_current_path = !Uberspark.Namespace.namespace_root_dir ^ "/" ^ Uberspark.Namespace.namespace_root ^ "/" ^ Uberspark.Namespace.namespace_config ^ "/" ^ Uberspark.Namespace.namespace_config_current in
	let config_ns_json_path = config_ns_path ^ "/" ^ Uberspark.Namespace.namespace_config_mf_filename in
	...
	load Uberspark.Namespace.namespace_config_current;
...
let remove 
	...
	if (config_name <> Uberspark.Namespace.namespace_config_default) then 
		begin
			let config_ns_path = !Uberspark.Namespace.namespace_root_dir ^ "/" ^ Uberspark.Namespace.namespace_root ^ "/" ^ Uberspark.Namespace.namespace_config ^ "/" ^ config_name in 
			let config_ns_json_path = config_ns_path ^ "/" ^ Uberspark.Namespace.namespace_config_mf_filename in
			...
			if config_hdr.name = config_name then
				ignore(switch Uberspark.Namespace.namespace_config_default);
			;
